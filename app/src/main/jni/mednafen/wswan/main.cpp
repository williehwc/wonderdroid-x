...
static short legacyStateFilenamePosIndex = 0;
static char* legacyStateFilename = (char*) malloc(20);
...
static void loadLegacyState() {
    ...
    FILE* file = fopen(legacyStateFilename, "r");
    if (file != NULL) {
        ...
        for (int i = 0; i < 14; i++) {
            unsigned *reg = (unsigned *) malloc(sizeof(unsigned));
        ...
        memcpy(&DMADest, toLoad + 65536 + sizeof(unsigned) * 14 + sram_size + 1319, 2);
...
{
 if (legacyStateFilenamePosIndex > 0) {
  char* toAppend = (char*) malloc(20);
  switch (legacyStateFilenamePosIndex) {
   case 1:
    sprintf(toAppend, "%d", cmd);
    break;
   case 2:
   case 3:
    sprintf(toAppend, "-%d", cmd);
   ...
   case 4:
   case 5:
    sprintf(toAppend, "_%d", cmd);
    break;
  }
  strcat(legacyStateFilename, toAppend);
  legacyStateFilenamePosIndex++;
  if (legacyStateFilenamePosIndex >= 6) {
   strcat(legacyStateFilename, ".sav");
   loadLegacyState();
   legacyStateFilenamePosIndex = 0;
  }
  free(toAppend);
  return;
 }
 switch(cmd)
 {
  ...
  case MDFN_MSC_RESET:
	...
	break;
  case 9:
    legacyStateFilenamePosIndex = 1;
    legacyStateFilename = (char*) MDFN_GetSettingS("filesys.path_sav").c_str();
    break;
 }
}
